...
During this exercise we will remove some duplication (DRY= Don't Repeat 
Yourself) from the test code we created earlier using higher order functions 
(functions that take other functions as parameters or return functions as their 
result). The code for this exercise is in the \texttt{fizzbuzz.scm} file.  
By choosing any of the available editors from the menu, it will be opened for you automatically.
...
invoke the \texttt{check} function such that calling this new function executes a single 
...
rewrite the tests as an application of \texttt{map} on a list of test cases.
You should use a \texttt{let} binding to avoid polluting the global name space.
...
\begin{enumerate}
  \item line 15: \texttt{null?} returns true if its parameter is the empty list
  \item line 17: \texttt{cadr} is a composition of \texttt{cdr} and \texttt{car} 
    \begin{equation*}
      \texttt{(cadr alist)} \Rightarrow \texttt{(car (cdr list))}
    \end{equation*}
  \item line 19: \texttt{eval} evaluates the given list in the same manner as the 
  ...
  \item line 23: \texttt{cddr} is a composition of \texttt{cdr} with itself
    \begin{equation*}
...
To accomplish the above task promote the \texttt{operator?} and 
\texttt{operator} functions to top level bindings and write separate test 
cases for these individual functions.
